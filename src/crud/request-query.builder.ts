...
import { hasValue, isArrayFull, isNil, isObject, isString, isUndefined } from './utils/checks.util';
...
export class RequestQueryBuilder {
    ...
    if (!isNil(j)) {
      ...
      this.queryObject[param] = [
        ...
        ...(Array.isArray(j) && !isString(j[0])
          ? (j as Array<QueryJoin | QueryJoinArr>).map((o: QueryJoin | [string, string[]]) => this.addJoin(o))
    ...
    if (!isNil(s)) {
      ...
      this.queryObject[param] = [
        ...
        ...(Array.isArray(s) && !isString(s[0])
          ? (s as Array<QuerySort | QuerySortArr>).map((o: QuerySort | [string, 'ASC' | 'DESC']) => this.addSortBy(o))
  ...
  cond(f: QueryFilter | QueryFilterArr, cond: 'filter' | 'or' | 'search' = 'search'): string {
    const filter: QueryFilter = Array.isArray(f) ? { field: f[0], operator: f[1], value: f[2] } : f;
  ...
  private addJoin(j: QueryJoin | QueryJoinArr): string {
    const join: QueryJoin = Array.isArray(j) ? { field: j[0], select: j[1] } : j;
    validateJoin(join);
    const d: string = this.options.delim;
  ...
  private addSortBy(s: QuerySort | QuerySortArr): string {
    const sort: { field: string; order: 'ASC' | 'DESC' } = Array.isArray(s) ? { field: s[0], order: s[1] } : s;
    validateSort(sort);
    const ds: string = this.options.delimStr;
  ...
  private setCondition(f: QueryFilter | QueryFilterArr | Array<QueryFilter | QueryFilterArr>, cond: 'filter' | 'or'): void {
    if (!isNil(f)) {
      const param: string = this.checkQueryObjectParam(cond, []);
      this.queryObject[param] = [
        ...this.queryObject[param],
        ...(Array.isArray(f) && !isString(f[0])
          ? (f as Array<QueryFilter | QueryFilterArr>).map((o: QueryFilter | QueryFilterArr) => this.cond(o, cond))
